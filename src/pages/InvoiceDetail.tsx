...
export default function InvoiceDetail() {
  ...
  async function capturePngBlob(): Promise<Blob | null> {
     ...
     * transform. Mobile transform:scale() was the cause of overlapping text.
     */
    const source = invRef.current;
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 no-print">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => nav(`/billing?edit=${invoice.id}`)}
            className="inline-flex items-center gap-1.5 bg-white border border-indigo-200 hover:bg-indigo-50 text-indigo-800 text-sm font-bold px-3.5 py-2 rounded-lg"
          >
